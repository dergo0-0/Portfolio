...
function MessengerIcon({ item }) {
  const key = messengerKey(item.href);
  const path = key ? ICON_PATHS[key] : null;
  if (!path) return null;
  return (
    <a
...
export default function Footer() {
  ...
  const messengers = links.filter((item) => messengerKey(item.href));
  const emails = links.filter((item) => !messengerKey(item.href));
  ...
  return (
    <footer className="footer" id="contact">
      ...
      {messengers.length > 0 && (
        <Reveal className="contact-icons" stagger={0.08} rotate={0}>
          {messengers.map((item) => (
            <MessengerIcon item={item} key={item.label} />
          ))}
        </Reveal>
      )}
